...
# 1.2. Maze structure
The robot is placed into multiply connected maze which means that simple algorithms as Wall Follower will be not able to handle it and we need to use more sophisticated algorithm as Tremaux's algorithm.
...
## 6. Implementation
# 6.1 Implementation
The main robot handling function is the function `next_move` receiving in the each step input from sensors and deciding on the next move.

The robot location with its heading is updated with each step and logged into the file, so robot movement in the maze can be later visualized.

I've implemented there 3 modes of operations. 1. `explore`, 2. `search`, 3. `race`.

# 6.1.1. The mode 1. explore
The robot starts in in the mode 1. `explore` where the goal is to drive through maze and create robot's internal map.

For exploration and decision about the next step is used `Tremaux's algorithm`. I've choosen Tremaux's algorithm for the reason that it's guaranteed to find solution in each maze and human could use this algorithm in the maze too.
The algorithm works in a following way:
 1. If you reach dead-end, turn around, start backtracking and mark the path as visited twice (=sign for robot to don't go there again)
 ...
 3. If you arrive at a junction that you have not visited yet, 
  if it has still unvisited paths choose an arbitrary unvisited path and follow it. If it was backtracking, set it to normal mode again. Otherwise if there are no unvisited paths, threat it as dead-end and execute step 1
 4. If you arrive at a junction that you have already visited, 
 if not backtracking threat it as dead-end and start backtracking. Otherwise if backtracking if it has still unvisited paths choose an arbitrary unvisited path and follow it. Set it from backtracking to normal mode again.
 if there are no unvisited paths anymore, continue bactracking following the way the robot originally entered the junction.

The main difficulty during implementation was for me to properly handle backtracking. Originally I've used simple method of returning to the previous node, but as maze is not simple-connected, in the case when it required return through multiple nodes the algorithm got stuck. 
So I implemented in addition tracking the path by which the robot came, so it can follow during the bactracking the opposite direction to its origin.

Before I've tried Wall follower algorithm, but this was not able to find its ways to the maze center as this maze is not simply connected.

# 6.1.2. The mode 2. search
When robot finish the exploration mode and maps the whole maze (which is in Tremaux's algorithm indicated by returning to innitial position),
...
# 6.1.2. The mode 3. race
After the shortest path is calculated, robot can enter into 3. `race mode` where he follows the actions previously calculated to get into the destination by the shortest path.


# 6.2. Refinement
To optimize the final score the robot will receive, after having working algorithm, I've enhanced racing mode so it fully utilize robot's ability to move by up to 3 fields in a given direction during one step, thus reducing total time it takes to robot to race into the destination.

## 7. Results
Using Tremaux's algorithm to create internal map of the maze in the first run (Exploration) and Djiskra's algorithm to find the shortest path in the second run (Racing)
